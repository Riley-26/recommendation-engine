					...
					<div className='min-h-screen mx-auto bg-gray-800 flex items-center justify-between flex-wrap max-w-5xl py-12'>
						{
							loggedContext.search.tracks.items.map((item:any, index:any) => {
								return (
									<div id={index} key={index} className='mx-12 my-8 flex items-center cursor-pointer w-2/5 rounded-lg hover:bg-gray-600 transition-all p-4' onClick={() => {newSearch(item.album.images[0].url, item.name, item.artists[0].name, index, item.id); window.location.href = `#${genre.toLowerCase()}`}}>
										<img className='w-32 rounded-lg' src={item.album.images[0].url} alt="" />
										<div>
											<h1 className='mx-4 my-2 text-2xl'>{item.name}</h1>
											<h2 className='mx-4 my-2 text-xl'>{item.artists[0].name}</h2>
										</div>
									</div>
								)
							})
